...
### Note: I really should figure out a way to save these commands in a chunk that isn't run because it takes too long. I'm sure that is a thing in markdown.
...
##### This will iterate through and make a nice figure for each
...
a <- ggplot(tmp, aes(x=morph, y=est_counts)) + geom_point(aes(size = 3, color = morph)) + scale_colour_manual(values = c("navyblue","forestgreen","tan3","dodgerblue"), guide = guide_legend(title = "Color morph", override.aes = list(size=4))) +  geom_smooth(method = loess) + ggtitle(paste0(gene, "\n(", transcript, "), q value = ", qval)) + guides(size=FALSE)
