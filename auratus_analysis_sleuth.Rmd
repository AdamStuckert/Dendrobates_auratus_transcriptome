...
xenann <- read.table("newallpep_tophit.txt", header = FALSE, fill = TRUE)
...
write.csv(GO, "genes4GOrilla.csv")
```
...
```{r}
colors <- read.csv("color_genes_3-26-18.csv")
...
# For loop that iterates through with description...

# for (i in 1:nrow(sig_colors)){
#  transcript <- sig_colors[i,"target_id"]
#  gene <- sig_colors[i,"ext_gene"]
#  qval <- sig_colors[i,"qval"]
#  desc <- sig_colors[i,"description"]
#  tmp <- sopop$obs_norm %>% dplyr::filter(target_id == transcript)  ### These are normalized values I think!
#  tmp <- dplyr::full_join(sopop$sample_to_covariates, tmp, by = 'sample')
#  tmp
  
  
#a <- ggplot(tmp, aes(x=morph, y=tpm)) + xlab("Morph") + ylab("Transcripts per million") + geom_jitter(aes(size = 3, color = morph), width = 0.2, height = 0) + scale_colour_manual(values = c("navyblue","forestgreen","tan3","dodgerblue"), guide = guide_legend(title = "Color morph", override.aes = list(size=4))) +  geom_smooth(method = loess) + ggtitle(paste0(gene, ", ", desc, "\n(", transcript, "), q value = ", qval)) + guides(size=FALSE) + theme_bw() + theme(panel.grid.major = element_blank())
...
# First download the Xenopus annotation from linux/enrique
xenopus <- read.table("newxen_tophit.txt", header = FALSE, fill = TRUE)
colnames(xenopus)[c(1:12)] <- c("transcript_id", "peptide_id", "percentage_id_matches",
                              "alignment_length", "number_mismatches", "number_gap_openings",
                              "query_start", "query_end", "alignment_start", "alignment_end",
                              "expected_value", "bitscore")


xenopus <- xenopus[,c(1:2)]
...
write.csv(pop_b_siggies, "auratus_blat_significant_genes.csv")

GOb <- dplyr::filter(pop_b_siggies, gene_name != "NA")
GOb <- dplyr::filter(GOb, gene_name != "")

write.csv(GOb, "xenopus_blat_genes4GOrilla.csv")
...
The below gets gene descriptions, so I can see what they actually are! I like the idea of this.
   
```{R}

atts <- listAttributes(xenopus)

g2desc <- biomaRt::getBM(attributes = c("ensembl_peptide_id_version", "ensembl_gene_id", "external_gene_name", "description"), mart = xenopus)  

desc <- g2desc[,3:4]
colnames(desc)[1] <- "ext_gene"

colors <- dplyr::left_join(colors, desc, by = "ext_gene")
write.csv(colors, "color_genes.csv")

```



This chunk is me trying to learn how to download all the relevant data from the internet for the Xenopus, bullfrog, and NR databases...

```{r}
upload <- list()
for (i in 1:nrow(fullanno)){
  search <- fullanno[i,"ensembl_transcript_id_version"]
  
tmp <- entrez_search(db = "protein", term = search, use_history = TRUE)
upload <- rbind(tmp, upload)
}
...
all2gene <- biomaRt::getBM(attributes = c("ensembl_peptide_id_version", "ensembl_transcript_id", "ensembl_gene_id", "external_gene_name"), mart = ensembl)



library(biomartr)
biomartr::listNCBIDatabases()
getMarts()
...
entrez_db_searchable("protein")
  # I think I want c("ACCN", "GENE", "UID")
...
all_links_together  <- entrez_link(db="gene", dbfrom="protein", id=ids)
all_links_together$links
all_links_together$links$protein_gene

eh <- entrez_fetch(db="gene", dbfrom="protein", id=ids, rettype = "native")
...
entrez_db_summary(db="protein")

allnr <- entrez_search(db="protein", term="")

nano <- entrez_search(db="protein", term="Nanorana parkeri")
nano

bull <- entrez_search(db="protein", term="Rana catesbeiana[ORGN]", use_history = TRUE)

bull
bull$file

bull_proteins <- entrez_fetch(db="protein", id=bull$id, rettype="fasta")





entrez_summary(db="protein", web_history=bull$web_history)

bull$web_history

for( seq_start in seq(1,200,50)){
    recs <- entrez_fetch(db="protein", web_history=bull$web_history,
                         rettype="ft", parsed = TRUE, retmax=50, retstart=seq_start)
    cat(recs, file="bull.xml", append=TRUE)
    cat(seq_start+49, "sequences downloaded\r")
}

nchar(recs)
cat(strwrap(substr(bull, 1, 1000)), sep="\n")
...
bullfrog <- entrez_fetch(db="protein", term="Rana catesbeiana", rettype = "xml")
bullfrog <- entrez_fetch()
