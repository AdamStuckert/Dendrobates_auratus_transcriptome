...
mheat <- plot_transcript_heatmap(sopop, mels$target_id, units = "est_counts", trans = "log") + labs(title = "Melanic genes") + theme(plot.title = element_text(hjust = 0)) + theme(legend.position="right") #+ scale_y_discrete(labels = mels$mel_gene_name) 
mheat
...
# I need a new way to add in the gene names!!!!! fuck.



iris <- read.csv("heatmapdocs/iridophore.csv")
iheat <- plot_transcript_heatmap(sopop, iris$target_id, units = "est_counts", trans = "log") + labs(title = "Iridophore genes") + theme(plot.title = element_text(hjust = 0)) + theme(legend.position="right") #+ scale_y_discrete(labels = iris$iridophore_gene_name)
iheat
melteddf <- read.csv("ggplotorder.csv")
colnames(melteddf)[3] <- "target_id"
